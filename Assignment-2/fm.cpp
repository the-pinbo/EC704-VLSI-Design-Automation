...
int main(int argc, char *argv[])
{
    ...
    if (inputfile.is_open())
    {
        ...
        cout << "unable to open the inputfile";
    ...
    {
        // Read the nodes and make partitions
        while (getline(nodesfile, line))
        {
            ...
            /*****update cell size*****/
            if (cell_map[node].cell_type.compare("FDRE") == 0)
            {
                cell_map[node].cell_size = 5;
            }
            else if (cell_map[node].cell_type.compare("LUT6") == 0)
            {
                cell_map[node].cell_size = 7;
            }
            else if (cell_map[node].cell_type.compare("LUT5") == 0)
            {
                cell_map[node].cell_size = 6;
            }
            else if (cell_map[node].cell_type.compare("LUT4") == 0)
            {
                cell_map[node].cell_size = 5;
            }
            else if (cell_map[node].cell_type.compare("LUT3") == 0)
            {
                cell_map[node].cell_size = 4;
            }
            else if (cell_map[node].cell_type.compare("LUT2") == 0)
            {
                cell_map[node].cell_size = 3;
            }
            else if (cell_map[node].cell_type.compare("LUT1") == 0)
            {
                cell_map[node].cell_size = 2;
            }
            else if (cell_map[node].cell_type.compare("CARRY8") == 0)
            {
                cell_map[node].cell_size = 34;
            }
            else if (cell_map[node].cell_type.compare("DSP48E2") == 0)
            {
                cell_map[node].cell_size = 429;
            }
            else if (cell_map[node].cell_type.compare("RAMB36E2") == 0)
            {
                cell_map[node].cell_size = 379;
            }
            else if (cell_map[node].cell_type.compare("BUFGCE") == 0)
            {
                cell_map[node].cell_size = 3;
            }
            else if (cell_map[node].cell_type.compare("IBUF") == 0)
            {
                cell_map[node].cell_size = 2;
            }
            else if (cell_map[node].cell_type.compare("OBUF") == 0)
            {
                cell_map[node].cell_size = 2;
            }
            ...
            if ((rand() % 2) == 1)
                ...
                partition = -1;
            // Adding nodes to partition and with the given constraint and updating the size
            if (partition == 0)
            {
                if ((sumB + cell_map[node].cell_size - sumA) <= 860)
                {
                    partition = ~partition;
                partB:
                    sumB = sumB + cell_map[node].cell_size;
                }
                else
                {
                    goto partA;
            ...
            else if (partition == -1)
            {
                if ((sumA + cell_map[node].cell_size - sumB) <= 860)
                {
                    partition = ~partition;
                partA:
                    sumA = sumA + cell_map[node].cell_size;
                }
                else
                {
                    goto partB;
    ...
    // read the nets and update the nodes
    if (netsfile.is_open())
    {
